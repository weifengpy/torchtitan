...
class TestParallelDimsMeshOperations(unittest.TestCase):
    ...
    def test_single_rank_mesh_operations(self):
        ...
        # dp_replicate mesh is always kept alive for mixed precision via replicate()
        self.assertIsNotNone(parallel_dims.get_optional_mesh("dp_replicate"))
...
class TestMixedPrecisionReplicate(DTensorTestBase):
    """Verify apply_replicate on Llama3 debugmodel matches single-device reference.

    Tests that torchtitan's apply_replicate with MixedPrecisionPolicy produces
    numerically identical results to a reference model with manually cast bf16
    parameters, following the pattern in
    ...
    @with_comms
    def test_apply_replicate_mixed_precision(self):
        """apply_replicate with bf16 on Llama3 debugmodel matches manual bf16 reference."""
        ...
        apply_replicate(
            model, dp_mesh, param_dtype=torch.bfloat16, reduce_dtype=torch.float32
        )
