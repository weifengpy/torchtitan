...
class TestFlexShardNumerics(FSDPTest):
    ...
    def test_flex_shard_vs_simple_fsdp(self):
        ...
        for step, (sf_loss, fs_loss) in enumerate(
            zip(simple_fsdp_losses, flex_shard_losses)
        ):
    ...
    def test_flex_shard_eager_vs_compiled(self):
        ...
        for step, (e_loss, c_loss) in enumerate(zip(eager_losses, compiled_losses)):
    ...
    def test_flex_shard_reshard_true_vs_false(self):
        """reshard_after_forward=True vs False: bitwise identical (hooks mode)."""
        self.init_test()
        model, inputs, labels = self.get_input()
        ...
        for step, (rt_loss, rf_loss) in enumerate(
            zip(reshard_true_losses, reshard_false_losses)
        ):
    ...
    def test_flex_shard_no_sync_grad_accumulation(self):
        """no_sync() gradient accumulation vs single batch (hooks mode)."""
        self.init_test()
        model, inputs, labels = self.get_input()

        # Model A: single fwd/bwd on doubled input
        ...
        optim_a = torch.optim.Adam(model_a.parameters(), lr=1e-4)
        optim_a.zero_grad()
        doubled_inputs = torch.cat([inputs, inputs], dim=0)
        doubled_labels = torch.cat([labels, labels], dim=0)
        out_a = model_a(doubled_inputs)
        loss_a = cross_entropy_loss(out_a, doubled_labels)
        loss_a.backward()
        optim_a.step()

        # Model B: two micro-batches with no_sync on first
        ...
        optim_b.zero_grad()
        with model_b.no_sync():
            out_b1 = model_b(inputs)
            loss_b1 = cross_entropy_loss(out_b1, labels)
            loss_b1.backward()
        out_b2 = model_b(inputs)
        loss_b2 = cross_entropy_loss(out_b2, labels)
        loss_b2.backward()
        optim_b.step()

        # Compare params after optimizer step
        for (n_a, p_a), (n_b, p_b) in zip(
            model_a.named_parameters(), model_b.named_parameters()
        ):
            assert torch.equal(p_a, p_b), f"Param {n_a}: single-batch != accumulated"
    ...
    def test_flex_shard_mixed_precision_vs_simple_fsdp(self):
        ...
        for step, (sf_loss, fs_loss) in enumerate(zip(sf_losses, fs_losses)):
    ...
    def test_flex_shard_flat_shard_vs_shard(self):
        ...
        for step, (s_loss, f_loss) in enumerate(zip(shard_losses, flat_losses)):
...
class TestFlexShardDoubleWrapping(FSDPTest):
    ...
    def test_double_wrap_convergence(self):
        ...
        flex_shard(model.experts, fsdp_mesh, shard_placement_fn={"*": Shard(0)})
        flex_shard(model, fsdp_mesh, shard_placement_fn={"*": Shard(0)})
...
class TestFlexShardNumericsFullModel(unittest.TestCase):
    ...
    Requires 8 GPUs (loss_compare.py default). Run with:
        pytest torchtitan/experiments/flex_shard/tests/test_numerics.py -x -k FullModel
    """
