...
def _reconstruct_full_tensor(
    ...
    # For DTensor params, also reconstruct across TP/EP mesh
    if info.get("is_dtensor"):
        from torch.distributed.tensor import DTensor
...
def _shard_for_load(
    ...
    For DTensor params, first shards along TP/EP dims, then along DP dim.
    """
    data = full_tensor
    placement = info["placements"][0]
    mesh = info["mesh"]

    # For DTensor params, shard along TP/EP dims first
    if info.get("is_dtensor"):
        from torch.distributed.tensor import DTensor
